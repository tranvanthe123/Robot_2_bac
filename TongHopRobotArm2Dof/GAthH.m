...
for pop_index=1:pop_size,                                                       %Khoi tao 
    ...
    sim('RobotArm.slx');                                                            %(tuc l cho lai ghep thu truoc khi
...
Kp10=par(bestchrom,1)-500;                               %cac 
Ki10=par(bestchrom,2)-500;                               %nhiem sac the
Kd10=par(bestchrom,3)-500;                               %cua cha me
Kp20=par(bestchrom,4)-500;                              %toi uu nhat
Ki20=par(bestchrom,5)-500;                               %de luu vao 
...
while ~Terminal, %Terminal se bang 1 neu chay du 200 the he hoac trong qua trinh chay co 1 the he con cai có bestfit thoa man sai so <epsilon
    ...
    for pop_index=1:pop_size,                                           %lan luot test tung ca the trong quan the
        Kp1=par(pop_index,1)-500;                                         %quy doi gi tri nhiem
        Ki1=par(pop_index,2)-500;                                         %sac the ve cac gia tri 
        Kd1=par(pop_index,3)-500;                                         %Kp1, Ki1, Kd1
        Kp2=par(pop_index,4)-500;                                         %Kp2,Ki2, Kd2
        Ki2=par(pop_index,5)-500;                                         %
        Kd2=par(pop_index,6)-500;                                         %
        sim('RobotArm.slx');                                             %tien hanh chay mo phong de kiem tra xem ca the nao toi uu
       ...
       % fitness(pop_index)=1/(J+eps);
        if length(e1)>9500                                                                          %neu mo phong chay duoc du 10000 mau(100s) thi
            ...
            Ki2
            Kd2
            J=(e1'*e1)+rho*(tau1'*tau1)+(e2'*e2)+rho*(tau2'*tau2);%+rho*(u'*u);        %tih J dua vao mau thu 8000 den 9500(giay thu 80
            %J=e4'*e4
...
Kp1=par(bestchrom,1)-500    %hien thi
Ki1=par(bestchrom,2)-500   %cac nhiem sac the 
Kd1=par(bestchrom,3)-500   %Kp1,Ki1, Kd1, Kp2,Ki2,Kd2
Kp2=par(bestchrom,4)-500    %cua ca the tim  duoc nho qua trinh chay 
Ki2=par(bestchrom,5)-500    %chuong trinh GA
Kd2=par(bestchrom,6)-500    %

J=1/bestfit(end)  %ham tieu chuan tuong ung ca the con tot nhat do
sim('RobotArm.slx');  %tien hanh mo phong lai de kiem tra ca the con tot nhat do cho dap ung he thong nhu the nao
